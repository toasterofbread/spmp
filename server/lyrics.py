    ...
    def getFurigana(self):
        ...
        def trimOkurigana(original: str, furigana: str) -> list:
            if (hasKanjiAndHiragana(original)):
                ...
                if trim_amount != 0:
                    return [
                        [original[:len(original) - trim_amount], furigana[:len(furigana) - trim_amount]],
...
class TimedLyrics(Lyrics):
    ...
    class Line:
        ...
        def __init__(self, data: dict, _index: int):
            ...
            for word_data in data["word"] if type(data["word"]) == list else [data["word"]]:
...
def findLyricsId(title: str, artist: str | None = None) -> int | None:
    params = {"title": title}
    if artist is not None:
        params["artist"] = artist

    response = requests.get(
        "https://petitlyrics.com/search_lyrics",
        params = params
    )
    response.raise_for_status()
